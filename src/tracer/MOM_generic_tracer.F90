      ...
      do
        if (g_tracer_is_prog(g_tracer)) then
          do k=1,nk ;do j=jsc,jec ; do i=isc,iec
            ...
            if (g_tracer%diag_id_boundary_forcing_tend .gt. 0) then !liao
          ...
          if (g_tracer%diag_id_boundary_forcing_tend .gt. 0) then !liao
             do k=1,nk ;do j=jsc,jec ; do i=isc,iec
                g_tracer%boundary_forcing_tend(i,j,k) =G%mask2dT(i,j)*(g_tracer%field(i,j,k,1) - g_tracer%boundary_forcing_tend(i,j,k))/dt
             enddo ; enddo ; enddo
          !liao
          endif
        endif
